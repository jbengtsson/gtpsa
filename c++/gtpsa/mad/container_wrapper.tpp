...
namespace gtpsa::mad {
   ...
   *
   *  This class is responsible to extract the tpsa_t pointers of the
   *  different
  ...
  public:
    inline GTPSA_CLASS
      (ContainerWrapper)(std::vector<GTPSA_CLASS(Wrapper) *> a_vec)
      : m_vec(a_vec) {}
    ...
    inline void rpminv
      (const GTPSA_CLASS(ContainerWrapper) &ma, std::vector<idx_t>& select) {
      /*
       * deduced from mad_tpsa.h
       */
      if (ma.size() != this->size()) {
  ...
  protected:
    inline void rapply2
      (const GTPSA_CLASS(ContainerWrapper) &ma,
       const GTPSA_CLASS(ContainerWrapper) &mb,
       void(*f)(ssz_t na, const GTPSA_PTR_T*a[], const GTPSA_PTR_T *b[],
		GTPSA_PTR_T *c[])) {
      auto tmpa = ma.getConstBufferPtrs();
      auto tmpb = mb.getConstBufferPtrs();
      auto tmpc = this->getBufferPtrs();
    ...
    inline std::vector<const GTPSA_PTR_T *> getConstBufferPtrs(void) const {
      ...
      std::transform
	(this->m_vec.begin(), this->m_vec.end(), vec.begin(),
	 [](const GTPSA_CLASS(Wrapper) *elem)
	 { return elem->getPtr(); }
	 );
      return vec;
    }
