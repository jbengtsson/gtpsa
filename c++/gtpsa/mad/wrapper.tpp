...
namespace gtpsa::mad {
  ...
  private:
    GTPSA_CLASS(LifeTimeManager)
      (const GTPSA_CLASS(LifeTimeManager) &o) = delete;

    GTPSA_CLASS(LifeTimeManager)
      &operator=(const GTPSA_CLASS(LifeTimeManager) &o) = delete;
  ...
  public:
    ...
    inline GTPSA_CLASS(Wrapper)(std::shared_ptr<desc> desc, const ord_t mo)
      : t_desc(desc),
      ltm(std::make_unique<GTPSA_CLASS(LifeTimeManager)>
	  (GTPSA_METH(newd)(desc->getPtr(), mo))) {}
    ...
    inline GTPSA_CLASS(Wrapper)(const GTPSA_CLASS(Wrapper) &o)
      ...
      , ltm(std::make_unique<GTPSA_CLASS(LifeTimeManager)>
	    (GTPSA_METH(new)(o.getPtr(), init::same)))
      { this->_copyInPlace(o); }
     ...
     *
     */
    inline void _copyInPlace(const GTPSA_CLASS(Wrapper) &o) {
      GTPSA_METH(copy)(o.getPtr(), this->getPtr()); }
    ...
    inline GTPSA_CLASS(Wrapper) newFromThis(void) const {
      return GTPSA_CLASS(Wrapper)(*this, init::same); }
     ...
     */
    inline auto uid(int32_t uid_)
    { return GTPSA_METH(uid)(this->getPtr(), uid_); }

    // inline auto uid(void) const
    // { return GTPSA_METH(uid)((P*)(this->getPtr()), 0); }
    ...
    inline void setnam(const std::string name)
    { GTPSA_METH(setnam)(this->getPtr(), name.c_str()); }

    /**
     * @brief:
     */
    void setvar
      (const GTPSA_BASE_T v, const idx_t iv_ = 0, const GTPSA_BASE_T scl_ = 0)
    {
     ...
     */
    inline auto idxs(const std::string s) const
    { return GTPSA_METH(idxs)(this->getPtr(), s.size(), s.data()); }
    ...
    inline auto cycle
      (const idx_t i, std::vector<ord_t>* m, GTPSA_BASE_T *v) const
    {
    ...
    inline auto geti(const idx_t i) const
    { return GTPSA_METH(geti)(this->getPtr(), i); }

    inline auto gets(const std::string s) const
    { return GTPSA_METH(gets)(this->getPtr(), s.size(), s.data()); }
     ...
     * @brief a*x[0]+b
     */
    inline void set0(const GTPSA_BASE_T a, const GTPSA_BASE_T b)
    { GTPSA_METH(set0)(this->getPtr(), a, b); }

    /**
     * @brief a*x[i]+b
     */
    inline void seti(const idx_t i, const GTPSA_BASE_T a, const GTPSA_BASE_T b)
    { GTPSA_METH(seti)(this->getPtr(), i, a, b); }

    /**
     * @brief a*x[m]+b
     */
    inline void sets
      (const std::string &s, const GTPSA_BASE_T a, const GTPSA_BASE_T b) {
     ...
     */
    inline void setm
      (const std::vector<ord_t> &m, const GTPSA_BASE_T a, const GTPSA_BASE_T b)
    {
     ...
     */
    inline auto setsm
      (const std::vector<int> m, const GTPSA_BASE_T a, const GTPSA_BASE_T b) {
    ...
    inline void print
      (const str_t name = nullptr, const num_t eps = 0, const int nohdr = 0,
       FILE *stream = nullptr) const {
      GTPSA_METH(print)(this->getPtr(), name, eps, nohdr, stream);
    }

    inline void pow
      (const GTPSA_CLASS(Wrapper) &a, const GTPSA_CLASS(Wrapper) &b) {
      this->process2w(a, b, GTPSA_METH(pow));
    }

    inline void pow(const GTPSA_CLASS(Wrapper) &a, const int n)
    { GTPSA_METH(powi)(a.getPtr(), n, this->getPtr()); }
     ...
     */
    inline void process1w
      (const GTPSA_CLASS(Wrapper) &a,
       void (*func)(const GTPSA_PTR_T *a, GTPSA_PTR_T *r)) {
      func(a.getPtr(), this->getPtr());
    }
    /*
     * @brief processing functions with two arguments in, single out, all
     * gtpsa object pointers
     */
    inline void process2w
      (const GTPSA_CLASS(Wrapper) &a, const GTPSA_CLASS(Wrapper) &b,
     ...
     */
    inline void process2w
      (const GTPSA_CLASS(Wrapper) &a, const GTPSA_BASE_T b,
    ...
    /**
     * @brief single argument in, single argument out, return argument
     * allocated
     */
    /**
    ...
    inline void taylor
      (const GTPSA_CLASS(Wrapper) &a, const std::vector<GTPSA_BASE_T> coeff,
       GTPSA_CLASS(Wrapper) *c) {
  ...
  protected:
    ...
    friend inline void
      process1to2w_
      (const GTPSA_CLASS(Wrapper) &a, GTPSA_CLASS(Wrapper) *r1,
     ...
     */
    friend inline void taylor
      (const GTPSA_CLASS(Wrapper) &a, std::vector<GTPSA_BASE_T> coeff,
       GTPSA_CLASS(Wrapper) *c);

    friend inline ord_t maxord
      (const std::vector<const GTPSA_CLASS(Wrapper) *>& vec);

  }; /* class GTPSA_CLASS(Wrapper) */

  /*
   * different mathematical functions processing the class processing one
   * input argument
   */
...
#define GTPSA_FUNC_ARG1_WITH_RET_ARG(fname)				\
  inline void fname (const GTPSA_CLASS(Wrapper)& t, GTPSA_CLASS(Wrapper)* r) \
  { r->process1w(t, GTPSA_METH(fname) ); }
...
#undef GTPSA_FUNC_ARG1
...
#if 0
  /*
   * @brief processing functions with a single argument in, single out, all
   * gtpsa object pointers
   */
  ...
  /*
   * @brief processing functions with two arguments in, single out, all gtpsa
   * object pointers
   */
  inline void process2w_
  (const GTPSA_CLASS(Wrapper) &a, const GTPSA_CLASS(Wrapper) &b,
   GTPSA_CLASS(Wrapper) *r,
   void (*func)(const GTPSA_PTR_T *a, const GTPSA_PTR_T *b, GTPSA_PTR_T *r)) {
   ...
   */
  inline void process2w_
  (const GTPSA_CLASS(Wrapper) &a, const GTPSA_BASE_T b, GTPSA_CLASS(Wrapper) *r,
   void (*func)(const GTPSA_PTR_T *a, const GTPSA_BASE_T b, GTPSA_PTR_T *r)) {
    func(a.getPtr(), b, r->getPtr());
  }

  /*
  inline GTPSA_CLASS(Wrapper) process2w
    (const GTPSA_CLASS(Wrapper)& a, const GTPSA_CLASS(Wrapper)&  b,
     void (*func)(const GTPSA_CLASS(Wrapper)& a, const GTPSA_CLASS(Wrapper)& b,
		  GTPSA_CLASS(Wrapper)* r) ) {
  ...
  */
  ...
  inline void sincos
  (const GTPSA_CLASS(Wrapper) &t, GTPSA_CLASS(Wrapper) *r1,
   GTPSA_CLASS(Wrapper) *r2) {
  ...
  inline ord_t maxord(const std::vector<const GTPSA_CLASS(Wrapper)*>& vec){
    ...
    std::transform
      (vec.begin(), vec.end(), tmp.begin(),
       [](const GTPSA_CLASS(Wrapper)* p) {return p->getPtr();});
    auto r =  GTPSA_METH(ordn)(tmp.size(), tmp.data());
    return r;
  }
  /*
  inline GTPSA_CLASS(Wrapper) add
    (const GTPSA_CLASS(Wrapper)& a, const GTPSA_CLASS(Wrapper)& b)
  { return process2w(a, b, add_wr); }
  inline GTPSA_CLASS(Wrapper) dif
    (const GTPSA_CLASS(Wrapper)& a, const GTPSA_CLASS(Wrapper)& b)
  { return process2w(a, b, dif_wr); }
  inline GTPSA_CLASS(Wrapper) sub
    (const GTPSA_CLASS(Wrapper)& a, const GTPSA_CLASS(Wrapper)& b)
  { return process2w(a, b, sub_wr); }
  inline GTPSA_CLASS(Wrapper) mul
    (const GTPSA_CLASS(Wrapper)& a, const GTPSA_CLASS(Wrapper)& b)
  { return process2w(a, b, mul_wr); }
  inline GTPSA_CLASS(Wrapper) div
    (const GTPSA_CLASS(Wrapper)& a, const GTPSA_CLASS(Wrapper)& b)
  { return process2w(a, b, div_wr); }
  */
