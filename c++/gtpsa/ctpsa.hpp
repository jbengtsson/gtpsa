...
class CTpsaTypeInfo : public GTpsaTypeInfo<ctpsa_t, cpx_t, ctpsa, mad::CTpsaWrapper, mad::_CTpsaWrapper, mad::_CTpsaContainerWrapper>  {};
   ...
   inline ctpsa(const ctpsa_bridge&& o)
	...
	inline auto get(void)                        const
      { return cpx_t_to_std_complex_double ( base::get()  ); }
	inline auto get(const idx_t i)               const
      { return cpx_t_to_std_complex_double ( base::get(i) ); }
	inline auto get(std::string s)               const
      { return cpx_t_to_std_complex_double ( base::get(s) ); }
	inline auto get(const std::vector<ord_t>& m) const
      { return cpx_t_to_std_complex_double ( base::get(m) ); }
