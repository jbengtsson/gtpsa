...
template<>
void gtpsa::ss_vect<double>::show
...
template void gtpsa::ss_vect<gtpsa::tpsa>::show
(std::ostream& strm, int level, bool with_endl) const;
template void gtpsa::ss_vect<gtpsa::ctpsa>::show
(std::ostream& strm, int level, bool with_endl) const;
...
arma::mat gtpsa::ss_vect<gtpsa::tpsa>::jacobian(void) const
{
  ...
  for(size_t slice = 0; slice < this->size(); ++slice) {
    ...
    for(size_t row = 0; row < nv; ++row){
      ...
      ord.at(row) += 1;
      if(t.index(ord) < 0){
	std::cerr << "Invalid index {" << int(ord.at(0)) << ", "
		  << int(ord.at(1)) << "," << int(ord.at(2)) << "}" <<std::endl;
	continue;
      }
...
void gtpsa::ss_vect<gtpsa::tpsa>::setJacobian(arma::mat& jac) {
  ...
  for(size_t slice = 0; slice < this->size(); ++slice) {
	...
	strm << "Invalid index {" << int(ord[0]) << ", " << int(ord[1]) << ","
	     << int(ord[2]) << "}" <<std::endl;
	throw std::runtime_error(strm.str());
	continue;
      }
...
arma::cube gtpsa::ss_vect<gtpsa::tpsa>::hessian() const
{
  ...
  for(size_t slice = 0; slice < this->size(); ++slice) {
	...
	ord[col] += 1;
	if(t.index(ord) < 0){
	  std::cerr << "Invalid index {" << int(ord[0]) << ", " << int(ord[1])
		    << "," << int(ord[2]) << "}" <<std::endl;
	  continue;
	}
...
void gtpsa::ss_vect<gtpsa::tpsa>::setHessian(arma::cube& jac)
{
  throw std::runtime_error
    ("setHessian not yet implemented for gtpsa::ss_vect<gtpsa::tpsa>");
}
...
{
  TpsaBridgeContainer<gtpsa::TpsaTypeInfo> bc
    (filter.as_const(this->state_space));
  return bc.getMaximumOrder();
}


template<>
void  gtpsa::ss_vect<gtpsa::tpsa>::rliebra
...
template<>
void gtpsa::ss_vect<gtpsa::tpsa>::fgrad
(gtpsa::tpsa * b, gtpsa::tpsa * r) const {
  const bridge_container_type mc_b(filter.as_const(this->state_space));
  mc_b.fgrad(b, r);
}
...
template<>
void  gtpsa::ss_vect<gtpsa::tpsa>::rcompose
(const gtpsa::ss_vect<gtpsa::tpsa>& a, const gtpsa::ss_vect<gtpsa::tpsa>& b)
{
  FilterBasePointers <gtpsa::tpsa, gtpsa::TpsaTypeInfo> filter;
...
template<>
void  gtpsa::ss_vect<gtpsa::tpsa>::rpminv
(const gtpsa::ss_vect<gtpsa::tpsa>& a, std::vector<idx_t>& select)
{
  FilterBasePointers <gtpsa::tpsa, gtpsa::TpsaTypeInfo> filter;
