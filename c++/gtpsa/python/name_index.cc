 ...
 */
#if 0
static void check_unique_keys_index
(const gpy::index_mapping_t& d, const std::string& info);
#endif
 ...
 */
#if 0
static void check_default_keys_index
(const gpy::index_mapping_t& d, const gpy::index_mapping_t& default_mapping,
 const std::string& info = "");
#endif
template<typename T>
static void check_unique_keys_index
(const std::map<const std::string, const T>& d, const std::string& info)
{
  ...
  if(keys.size() != d.size()) {
    ...
    strm << prefix << " " << info << ": found " << keys.size()
	 << " unique keys," << " but have "<< d.size()
	 << " entries in the index map " << suffix;
    throw std::runtime_error(strm.str());
  }

  if(indices.size() != d.size()) {
    std::stringstream strm;
    strm << prefix << " " << info << ": found " << indices.size()
	 << " unique indices," << " but have "<< d.size()
	 << " entries in the index map " << suffix;
    throw std::runtime_error(strm.str());
  }
  ...
  for(const auto& [key, index]: default_mapping) {
    ...
    if(check->second != index) {
      ...
      strm << prefix << " " << info << " : found that for key " << key
	   << " an index of " << check->second
	   << " was found, but it should be " << " of value " << index;
      throw std::runtime_error(strm.str());
    }
  }
...
template<typename T>
static std::vector<std::string> mapping_keys_for_dir
(const std::map<const std::string, T>& im)
{
...
template<typename T>
static auto mapping_index
(const std::map<const std::string, T>& im, const std::string& key)
{
 ...
 */
static const gpy::index_mapping_t from_hamiltion_index_mapping
(const gpy::hamiltonian_pair_mapping_t& d)
{
...
gpy::IndexMapping::IndexMapping
(const index_mapping_t& d, const std::string info)
  : IndexMappingBase()
  , m_mapping(d)
  , m_info(info)
{
...
std::vector<size_t> gpy::IndexMapping::order_vector_from_power
(const index_mapping_t& powers) const
{
  std::vector<size_t> p(this->m_mapping.size());
  std::transform(p.begin(), p.end(), p.begin(), [](const size_t val)
  {return 0;});
...
gpy::HamiltonianIndexMapping::HamiltonianIndexMapping
(const hamiltonian_pair_mapping_t& d, const std::string info)
  : IndexMappingBase()
...
size_t  gpy::HamiltonianIndexMapping::index(const std::string key) const
{
  throw std::runtime_error
    ("Hamiltoninan index mapping needs to be implemented");
  // return mapping_index(this->m_hamiltonian_mapping, key);
}


namespace gtpsa::python {
  const IndexMapping DefaultIndexMapping
  (gpy::default_mapping, "gtpsa::ss_vect");
  std::shared_ptr<IndexMapping> default_index_mapping_ptr =
    std::make_shared<IndexMapping>(gpy::default_mapping, "gtpsa::ss_vect");
