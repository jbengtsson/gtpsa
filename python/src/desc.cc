...
Args:\n\
    ...
    po: maximum order (of parameters)?\n\
";
  py::class_<gtpsa::mad::desc, std::shared_ptr<gtpsa::mad::desc>> desc
    (m, "desc" );
  desc
    // This function is for mapping data fields - not function calls;
    // i.e., the C++ wrapper to the CERN gtpsa library is using utility
    // functions.
    ...
    .def("number_of_variables", &gtpsa::mad::desc::getNv  )
    .def("maximum_orders",      &gtpsa::mad::desc::maxOrd )
    .def("maximum_length",      &gtpsa::mad::desc::maxLen )
    // rename this mehtod
    ...
    .def("isvalid"        , py::overload_cast<const std::vector<ord_t>&>
	 ( &gtpsa::mad::desc::isvalid, py::const_ ))
    // For general indexing - see src/gtpsa/python/src/ss_vect.cc: idx().
    .def("index"          , py::overload_cast<const std::vector<ord_t>&>
	 ( &gtpsa::mad::desc::idx,     py::const_ ))
    ...
    .def(py::init<int, ord_t, int, ord_t, std::vector<ord_t>>(),
	 desc_newvpo_doc, py::arg("nv"), py::arg("mo"), py::arg("np"),
	 py::arg("po"), py::arg("ord_vec"));
}
